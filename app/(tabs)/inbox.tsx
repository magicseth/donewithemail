...
const TRIAGE_CONFIG = {
  ...
  // Y position where the triage line sits (where ball starts)
  get triageLineY() { return this.rowHeight; },
  // How far targets are from center
  targetOffsetX: 60,
  // Ball size
  ballSize: 32,
  // How much the ball moves relative to finger movement
  ballTravelMultiplier: 1.5,
  // Distance threshold for "near target" detection (scales ball)
  proximityThreshold: 50,
  // Proximity threshold for activation (0-1, triggers color change)
...
function useCreateTriageState(): TriageState {
  const { rowHeight, listTopPadding, targetOffsetX, ballTravelMultiplier, proximityThreshold, activationThreshold } = TRIAGE_CONFIG;
  const CENTER_X = SCREEN_WIDTH / 2;
  const TARGET_LEFT_X = CENTER_X - targetOffsetX;
  const TARGET_RIGHT_X = CENTER_X + targetOffsetX;
  ...
  const topRowIndex = useDerivedValue(() => {
    // With listTopPadding, at scrollY=0, first row is below triage line
    // At scrollY=listTopPadding, first row is at triage line
    // At scrollY=listTopPadding+rowHeight, second row is at triage line
    const adjustedScroll = scrollY.value;
    return Math.max(0, Math.floor(adjustedScroll / rowHeight));
  });
  ...
  const leftProximity = useDerivedValue(() => {
    const distance = Math.abs(ballX.value - TARGET_LEFT_X);
    return Math.max(0, 1 - distance / proximityThreshold);
  });

  const rightProximity = useDerivedValue(() => {
    const distance = Math.abs(ballX.value - TARGET_RIGHT_X);
    return Math.max(0, 1 - distance / proximityThreshold);
  });

  const isActivated = useDerivedValue(() => {
    return leftProximity.value > activationThreshold || rightProximity.value > activationThreshold;
  });

  const direction = useDerivedValue(() => {
    // 0 = done (left), 1 = reply (right)
    // Default to done, switch to reply if closer to right
    const delta = fingerX.value - startX.value;
    return delta > 20 ? 1 : 0;
  });
...
// Derived values from config (module-level for use in styles)
const CENTER_X = SCREEN_WIDTH / 2;
const TARGET_LEFT_X = CENTER_X - TRIAGE_CONFIG.targetOffsetX;
const TARGET_RIGHT_X = CENTER_X + TRIAGE_CONFIG.targetOffsetX;
...
const TriageOverlay = React.memo(function TriageOverlay() {
  ...
  const { rowHeight, triageLineY, ballSize, activationThreshold } = TRIAGE_CONFIG;

  // Ball position and scale
  const ballStyle = useAnimatedStyle(() => {
    const halfBall = ballSize / 2;
    // Y follows scroll - ball sits at triage line, moves up as row scrolls
    const scrollOffset = triage.scrollY.value % rowHeight;
    const ballY = triageLineY - scrollOffset + TRIAGE_CONFIG.listTopPadding;
  ...
  const leftTargetBgStyle = useAnimatedStyle(() => {
    const isActive = triage.leftProximity.value > activationThreshold;
    return {
  ...
  const rightTargetBgStyle = useAnimatedStyle(() => {
    const isActive = triage.rightProximity.value > activationThreshold;
    return {
      backgroundColor: isActive
        ? "#6366F1"
...
const AnimatedRowWrapper = React.memo(function AnimatedRowWrapper({ index, children }: AnimatedRowWrapperProps) {
  const triage = useTriageState();
  const { activationThreshold } = TRIAGE_CONFIG;
  ...
  const rowStyle = useAnimatedStyle(() => {
    ...
    // Not at triage line - transparent (use default)
    return { backgroundColor: "transparent", borderLeftWidth: 0, borderLeftColor: "transparent" };
  });
...
export default function InboxScreen() {
  ...
  // For backward compat with triage action (need direction as React state for mutation)
  const [triageDirection, setTriageDirection] = useState<"done" | "reply_needed">("done");

  // Sync direction from shared value to React state (only for the mutation call)
  useAnimatedReaction(
    () => triageState.direction.value,
    (dir, prevDir) => {
      if (dir !== prevDir) {
        runOnJS(setTriageDirection)(dir === 0 ? "done" : "reply_needed");
      }
    }
  );
  ...
  const handleAutoTriage = useCallback(async (emailId: string) => {
    ...
    // Get the triage action based on current direction from shared state
    const action = triageDirection;

    console.log(`[AutoTriage] Called for: "${subject}" - Action: ${action}`);
    ...
    // Haptic feedback - different for each action
    if (Platform.OS !== "web") {
      if (action === "done") {
    ...
    console.log(`[AutoTriage] Triaging to '${action}': "${subject}"`);
    try {
      await triageEmail({
        emailId: emailId as Id<"emails">,
        action,
      });
      console.log(`[AutoTriage] Success: "${subject}" -> ${action}`);

      // Reset ball to center
      triageState.fingerX.value = CENTER_X;
      triageState.startX.value = CENTER_X;
    } catch (error) {
  ...
  }, [triageEmail, triagedIds, emails, triageDirection, triageState]);
  ...
  // Viewability callback for auto-triage (when items scroll off top)
  ...
  // Track scroll direction and update triage state
  const handleScroll = useCallback((event: any) => {
    ...
    triageState.scrollY.value = currentOffset;
  }, [triageState.scrollY]);
    ...
    <TriageContext.Provider value={triageState}>
      <GestureHandlerRootView style={styles.container}>
        ...
        <View style={styles.swipeHintContainer}>
          <Text style={styles.swipeHintText}>Scroll to triage • Swipe left for TODO</Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  // Grey until ball hits target
  triageLinePending: {
    backgroundColor: "#F3F4F6",
    borderLeftWidth: 4,
    borderLeftColor: "#9CA3AF",
  },
  // Green when ball activated and direction is done
  triageLineActiveDone: {
    backgroundColor: "#ECFDF5",
    borderLeftWidth: 4,
    borderLeftColor: "#10B981",
  },
  // Purple when ball activated and direction is reply
  triageLineActiveReply: {
    backgroundColor: "#EEF2FF",
    borderLeftWidth: 4,
    borderLeftColor: "#6366F1",
  },
  // Next row fades to grey
  triageLineNext: {
