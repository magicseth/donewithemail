...
// Helper to extract attachments from Gmail message parts
function extractAttachments(parts: any[], attachments: AttachmentInfo[] = []): AttachmentInfo[] {
...
// Get email attachments metadata from Gmail
export const getEmailAttachments = action({
  args: {
    userEmail: v.string(),
    emailExternalId: v.string(),
  },
  handler: async (ctx, args): Promise<AttachmentInfo[]> => {
    ...
    const user: UserData | null = await ctx.runQuery(internal.gmailSync.getUserByEmail, {
    ...
    // Fetch the email metadata from Gmail
    const response = await fetch(
      `https://gmail.googleapis.com/gmail/v1/users/me/messages/${args.emailExternalId}?format=full`,
      {
    ...
    if (data.payload?.parts) {
      extractAttachments(data.payload.parts, attachments);
    }
...
export const downloadAttachment = action({
  ...
  handler: async (ctx, args): Promise<{ data: string; mimeType: string }> => {
    // Get user with Gmail tokens
    type UserData = {
      _id: Id<"users">;
      email: string;
    ...
    const response = await fetch(
      `https://gmail.googleapis.com/gmail/v1/users/me/messages/${args.emailExternalId}/attachments/${args.attachmentId}`,
      {
        headers: { Authorization: `Bearer ${accessToken}` },
      }
    );
    ...
    return {
      ...
      mimeType: "application/octet-stream", // Actual mime type should be tracked from getEmailAttachments
    };
  },
});
